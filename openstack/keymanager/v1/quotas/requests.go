...
func Get(ctx context.Context, client *gophercloud.ServiceClient) (r GetResult) {
	resp, err := client.Get(ctx, getURL(client), &r.Body, nil)
...
func GetProjectQuota(ctx context.Context, client *gophercloud.ServiceClient, projectID string) (r GetProjectResult) {
	resp, err := client.Get(ctx, getProjectURL(client, projectID), &r.Body, nil)
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
	return
}
...
// values provided.
func Update(ctx context.Context, c *gophercloud.ServiceClient, projectID string, opts UpdateOptsBuilder) (r gophercloud.Result) {
	b, err := opts.ToQuotaUpdateMap()
	if err != nil {
		r.Err = err
		return
	}
	resp, err := c.Put(ctx, updateProjectURL(c, projectID), b, &r.Body, &gophercloud.RequestOpts{
		OkCodes: []int{204},
	})
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
	return
...
func Delete(ctx context.Context, c *gophercloud.ServiceClient, projectID string) (r gophercloud.Result) {
	resp, err := c.Delete(ctx, deleteProjectURL(c, projectID), &gophercloud.RequestOpts{
		OkCodes: []int{204},
	})
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
	return
